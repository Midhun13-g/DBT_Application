...
public interface CommunityActionRepository extends JpaRepository<CommunityAction, Long> {
    ...
    @Query("SELECT ca FROM CommunityAction ca WHERE ca.isActive = true AND " +
           "(:district IS NULL OR ca.district = :district) AND " +
           "(:actionType IS NULL OR ca.actionType = :actionType) AND " +
           "(:targetAudience IS NULL OR ca.targetAudience = :targetAudience) AND " +
           "(:keyword IS NULL OR LOWER(ca.title) LIKE LOWER(CONCAT('%', :keyword, '%')) OR LOWER(ca.description) LIKE LOWER(CONCAT('%', :keyword, '%'))) " +
